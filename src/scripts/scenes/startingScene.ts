export default class startingScene extends Phaser.Scene {
  ...
  create() {
    this.add.image(0, 0, 'background').setOrigin(0).setScale(0.21)
    this.sound.play('main', { loop: true })

    this._startButton = this.add.image(150, 140, 'startButton').setOrigin(0).setScale(0.2)
    this._startButton.setInteractive()

    const text = this.add.text(170, 130, 'loading...', {
      fontFamily: 'Arial',
      fontSize: '16px',
      color: '#ffffff',
      fontStyle: 'bold'
